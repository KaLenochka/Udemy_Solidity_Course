...
/*
Cybersecurity in solidity - against the bad actors, accidential occurances
Withdrawal pattern that direct tracsfer call is not mode which pases a security threat.
...
return true;
 ...
 What could go wrong&
What happen if Tatiana rejects the tracsfer? If Tatiana rejects the transfer
the function is going to fail and then John will not get paid !!!

Thinking that all contracts and accounts want to accept funds automaticaly may be 
intuitive at first but it is in fact very naive and very dengerous to build your code like this
...
//if one contract has default fallback function it would be deadly fot the naive contract 
...
//No! Because not all contracts will fail or a mailcious - it would deter DAOs
...
contract Attacker {
    ...
    //well if you call the address from the constructer there are no byte codes 
    //And what we will do?? THE WITHDRAWAL PATTERN!!!

...
/*
A cryptographic hash function (CHF) is the mathematical algorythm that maps data of arbitrary size
(often called the "message") to a bit array of a fixed size (the "hash value", "hash" or "message digest")/

It is a one way function, that is a function, which is practically infeasible to inverse the computation.
...
ripemd160(bytes memory) returns (bytes20) - compute RIPEMD160 hash of the input;

Keccak is a leading hashing function desined by non-NSA disigners. Keccak won NIST competition to become the official sha3
...
contract Oracle {
    ...
    function setRand(uint _randData) external {
...
contract GenerateRandomNumber {
    ...
    //modulo (%) - so by computing against the remainder we will be able to produce a random number within the range
    ...
    function randMode(uint range) external view returns (uint) {

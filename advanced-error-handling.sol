...
A way of tackling this is that when the error happens, the state reverts back to its original state.
Below are some of the important methods fot error hadling:

    assert(bool condition) - In case condition is not met, this method calls causes an 
    ...
    require(bool condition, string memory message) - In case condition is not met, this method call reverts to the original state.
...
contract Vendor {
    ...
    modifier onlySeller() {
        require(seller == msg.sender, 'Only owner can be seller! ');
        _;
    }
